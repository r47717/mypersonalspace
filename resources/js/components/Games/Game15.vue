...
export default {
    ...
    computed: {
        ...
        canMoveLeft() {
            return this.cells.find(
                ({ row, col }) =>
                    row === this.emptyCell.row && col === this.emptyCell.col + 1
            );
        },
        canMoveRight() {
            return this.cells.find(
        ...
        canMoveUp() {
            return this.cells.find(
                ({ row, col }) =>
                    col === this.emptyCell.col && row === this.emptyCell.row + 1
            );
        },
        canMoveDown() {
            return this.cells.find(
    ...
    methods: {
        ...
        onClick(cell) {
            if (cell === this.canMoveRight)
                this.exchange(this.canMoveRight, this.emptyCell);
            else if (cell === this.canMoveLeft)
                this.exchange(this.canMoveLeft, this.emptyCell);
            else if (cell === this.canMoveUp)
                this.exchange(this.canMoveUp, this.emptyCell);
            else if (cell === this.canMoveDown)
                this.exchange(this.canMoveDown, this.emptyCell);
        },
